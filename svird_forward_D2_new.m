...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 theta(1)=0.557218630846905;  theta(2)=0.094857496888616;  theta(3)=0.218797558713388;  theta(4)=-0.042367923194980;
  theta(5)=-0.350680911798113;  theta(6)=-0.103743715412068;  theta(7)=0.088725461392437;
  theta(8)=0.022075713972325;  theta(9)=-0.003679718568528;  theta(10)=-0.008291823901679;


% Y0(1) = (N - 3942002 - 3580 - (1139396-21486))/N;
Y0(1) = N - 20086693 - 25039;
Y0(2) = 2086693;
...
%%%%%%%%%%%%%%%%%% GENERATE ACTUAL INCIDENCE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Idata(i) = Idata_clean(i) + normrnd(0,30000);
    Ddata(i) = Ddata_clean(i) + normrnd(0,50);
    if Idata(i) < 0
...
SynthDI2R = [tdata' Idata Ddata D];
writematrix(SynthDI2R,'SynthDI2R.txt','Delimiter','tab')
