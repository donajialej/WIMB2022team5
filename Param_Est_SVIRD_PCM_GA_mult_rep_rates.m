function Param_Est_SVIRD_PCM_GA_mult_rep_rates
...
global N alpha p gammasd gammasr gammavr gammavd delta1 delta2 m a b Y0 n K tdata scale1 scale2 scale3 rep_rate_D 
...
save('fits/GA_repr.mat','rp','NumCurves','rep_rate','tcalendar')
%%
clear
close all
load('fits/GA_repr.mat')

figure10 = figure;
hold on
randColor = [208,209,230; 166,189,219; 116,169,207; 90,169,207; 78,179,211; 66,146,198; 2,129,138;  33,113,181; 8,81,156; 8,48,107]/255;
% cm = parula(NumCurves);
for it = 1:NumCurves
    txt=['Report ',num2str(rep_rate*it*100), '%'];
    line10x = plot(tcalendar, rp(:,it),'Color',randColor(it,:),'DisplayName',txt,'LineWidth',1);
    hold on
end
line5x = yline(1,':','LineWidth',1,'HandleVisibility', 'off');
ax = gca; 
ax.FontSize = 6; 
xlabel('\fontsize{8}Time (days)');
ylabel('\fontsize{8}R_e(t)')

leg=legend('FontSize',8,'Location','best');
...
saveas(gcf,'figures/GA_report','epsc')
savefig('figures/GA_report')
