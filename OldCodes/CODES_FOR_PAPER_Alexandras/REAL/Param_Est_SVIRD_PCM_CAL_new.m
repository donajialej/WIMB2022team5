function Param_Est_SVIRD_PCM_CAL_new
...
% line3333 = plot(tcalendar,beta(tdata,theta0,m,a,b),'b');
% set(line3333,'Linewidth',2)
% hold on
...
% ylim([max(min(min(VC)),0)-10 max(max(SC))+10])
xlim([datetime(2021,07,9) datetime(2021,11,25)])

legend([line10A(1) line2aA(1) line10AB(1) line2aAB(1)],{'Reconstructed S(t)','Mean of Reconstructed S(t)',...
...
legend([line10ABC(1) line2aABC(1) line10ABCD(1) line2aABCD(1)],{'Reconstructed I_s(t)', 'Mean of Reconstructed I_s(t)',...
...
% ylim([max(min(min(Ddata)),0)-1 max(max(Ddata))+1])
xlim([datetime(2021,07,9) datetime(2021,11,25)])
...
legend([line2x(1) line10x(1)],{'Mean of Reconstructed Effective Reproduction Number',... 
    'Reconstructed Effective Reproduction Numbers'},'FontSize',18,'Location','best');

xlabel('\fontsize{18}Time (days)');
ylabel('\fontsize{18}\mathcal{R}_e(t)')
