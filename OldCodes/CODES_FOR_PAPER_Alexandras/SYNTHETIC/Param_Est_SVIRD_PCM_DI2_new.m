function Param_Est_SVIRD_PCM_DI2_new
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 theta_model(1)=0.557218630846905;  theta_model(2)=0.094857496888616;  theta_model(3)=0.218797558713388;
 theta_model(4)=-0.042367923194980;
  theta_model(5)=-0.350680911798113;  theta_model(6)=-0.103743715412068;  theta_model(7)=0.088725461392437;
  theta_model(8)=0.022075713972325;  theta_model(9)=-0.003679718568528;  theta_model(10)=-0.008291823901679;
...
curvesI = []; curvesD = []; curves_results = [];  bt = []; rp = [];
SC = []; VC = []; ISC = [];  IVC = []; DC = [];
...
nmb = nmb + 1;
    ...
    for t = 1:K
        tau1 = abs(Idata(t));
        yirIdata(t,1) = poissrnd(tau1,1,1);
        tau2 = abs(Ddata(t));
        yirDdata(t,1) = poissrnd(tau2,1,1);
    end
    
    curvesI = [curvesI (yirIdata)];
    curvesD = [curvesD (yirDdata)];

    ExpIncData = curvesI(1:K,iter);
    ExpCumDeaths = cumsum(curvesD(1:K,iter));
    ExpIncDeaths = curvesD(1:K,iter);
...
for lp = 1:num_it  
    ...
    [~,~,~,~,~,~,jacobian] = lsqnonlin(@(theta) G(theta,u,ExpCumDeathsN),theta,[],[],options1);
    
    G_prime_theta = jacobian;
    
    [~,~,~,~,~,~,jacobian] = lsqcurvefit(@(theta,tdata) B(theta,u,ExpCumDeathsN),theta,tdata,DataN,[],[],options2);
...
% lambda = lambda0/exp(factor*lp);
    ...
    step_theta = -(G_prime_theta'*G_prime_theta + B_prime_theta'*B_prime_theta + lambda*eye(m))\...
        (G_prime_theta'*G(theta,u,ExpCumDeathsN)+ scale3*B_prime_theta'*(B(theta,u,ExpCumDeathsN) - DataN) + lambda*(theta - theta0));
    theta = step_theta + theta;
    
    [~,~,~,~,~,~,jacobian] = lsqnonlin(@(u) G(theta,u,ExpCumDeathsN),u,[],[],options1);
    
    G_prime_u = jacobian;
    
    [~,~,~,~,~,~,jacobian] = lsqcurvefit(@(u,tdata) B(theta,u,ExpCumDeathsN),u,tdata,DataN,[],[],options2);
    
    B_prime_u = jacobian;
    
    step_u = -(G_prime_u'*G_prime_u + B_prime_u'*B_prime_u)\(G_prime_u'*G(theta,u,ExpCumDeathsN)...
        + scale3*B_prime_u'*(B(theta,u,ExpCumDeathsN) - DataN));
    u = step_u + u;
...
y1 = round(N.*y(1,tdata,u));
y2 = round(N.*y(2,tdata,u));
y3 = round(N.*y(3,tdata,u));
y4 = round(N.*y(4,tdata,u));
...
yd = round(rep_rate_D*N.*(gammasd.*y(3,tdata,u) + gammavd.*y(4,tdata,u)));

yc = round(rep_rate_C*N.*(beta(tdata,theta).*y(1,tdata,u).*(y(3,tdata,u) + y(4,tdata,u))./(1-ExpCumDeathsN)...
          + (1 - alpha).*beta(tdata,theta).*y(2,tdata,u).*(y(3,tdata,u) + y(4,tdata,u))./(1-ExpCumDeathsN)));
      
repr = beta(tdata,theta).*y(1,tdata,u)./((1-ExpCumDeathsN).*(gammasr + gammasd))...
          + (1 - alpha).*beta(tdata,theta).*y(2,tdata,u)./((1-ExpCumDeathsN).*(gammavr + gammavd));  
...
curves_results = [curves_results (yc)];
...
rpmodel = beta(tdata,theta_model).*(Y(:,1)/N)./((1-ExpCumDeathsN).*(gammasr + gammasd))...
          + (1 - alpha).*beta(tdata,theta_model).*(Y(:,2)/N)./((1-ExpCumDeathsN).*(gammavr + gammavd)); 
...
line333 = plot(tcalendar,beta(tdata,theta_model),'r');
...
xlim([datetime(2021,07,9) datetime(2021,11,25)])
ylim([max(min(min(Idata)),0)-10 max(max(curves_results))+100000])
...
ylim([max(min(min(Ddata)),0)-1 max(max(Ddata))+100])
xlim([datetime(2021,07,9) datetime(2021,11,25)])
...
dydt = zeros(6,1);
dydt(1) = -beta(t,theta).*y(1).*(y(3)+y(4))./(N - y(6)) - p.*y(1) + delta1*y(5) + delta2*y(2);
dydt(2) = p.*y(1) - (1-alpha)*beta(t,theta).*y(2).*(y(3)+y(4))./(N - y(6)) - delta2*y(2);
dydt(3) = beta(t,theta).*y(1).*(y(3)+y(4))./(N - y(6)) - (gammasr + gammasd)*y(3);
dydt(4) = (1-alpha).*beta(t,theta).*y(2).*(y(3)+y(4))./(N - y(6)) - (gammavr + gammavd)*y(4);
dydt(5) = gammasr.*y(3) + gammavr.*y(4) - delta1*y(5);
dydt(6) = gammasd.*y(3) + gammavd.*y(4);
end


function g = G(theta,u,D)
...
g(1:K,1) = dydt(1,tdata,u) - (-beta(tdata,theta).*y(1,tdata,u).*(y(3,tdata,u)+y(4,tdata,u))./(1-D)...
    - p.*y(1,tdata,u) + delta1*yR + delta2*y(2,tdata,u));

g(K+1:2*K,1) = dydt(2,tdata,u) - (p.*y(1,tdata,u) - (1-alpha)*beta(tdata,theta).*y(2,tdata,u).*(y(3,tdata,u)...
    + y(4,tdata,u))./(1-D) - delta2*y(2,tdata,u)); 

g(2*K+1:3*K,1) = dydt(3,tdata,u) - (beta(tdata,theta).*y(1,tdata,u).*(y(3,tdata,u)+y(4,tdata,u))./(1-D)...
     - (gammasr + gammasd)*y(3,tdata,u));

g(3*K+1:4*K,1) = dydt(4,tdata,u) - ((1-alpha).*beta(tdata,theta).*y(2,tdata,u).*(y(3,tdata,u)+y(4,tdata,u))./(1-D)...
     - (gammavr + gammavd)*y(4,tdata,u));
...
function b = B(theta,u,D)
global  tdata alpha gammasd gammavd K scale1 rep_rate_D rep_rate_C
    
  b(1:K,1) =  rep_rate_C*((y(1,tdata,u) + y(2,tdata,u).*(1 - alpha)).*beta(tdata,theta).*(y(3,tdata,u) + y(4,tdata,u))./(1-D));
  b(K+1:2*K,1) = rep_rate_D*scale1*(gammasd.*y(3,tdata,u) + gammavd.*y(4,tdata,u));
end
...
dy(1) = -beta(t,theta).*y(1).*(y(3)+y(4))./(1-Dsp) - p.*y(1) + delta1*y5 + delta2*y(2);
dy(2) = p.*y(1) - (1-alpha)*beta(t,theta).*y(2).*(y(3)+y(4))./(1-Dsp) - delta2*y(2);
dy(3) = beta(t,theta).*y(1).*(y(3)+y(4))./(1-Dsp) - (gammasr + gammasd)*y(3);
dy(4) = (1-alpha).*beta(t,theta).*y(2).*(y(3)+y(4))./(1-Dsp) - (gammavr + gammavd)*y(4);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function yy = u0f(theta,D)
global n
...
for i = 1:4
    for  j = 1:n
           yy(j+(i-1)*n,1) = (2*(j-1)+1)*integral(@(x)fun(x,j,i,theta,D),-1,1)/2;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = fun(x,j,i,theta,D)
global a b Y0 

t = ((b-a).*x+a+b)./2;

[T,Y] = ode23s(@(t,y) svird(t,y,theta,D),[a b],Y0);
