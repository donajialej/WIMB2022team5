function predictor_corrector_SIRD_Spain
...
% load SFcumcases.txt;
% load Mcumcases.txt;
% load Georgiacumcases_full.txt;
% load Nethcumcases.txt;
load Spaincumcases.txt;
...
% Cdata = Mcumcases(:,2);
% tdata = Georgiacumcases_full(:,1);
% Cdata = Georgiacumcases_full(:,2);
% tdata = Nethcumcases(:,1); % alpha0 = 1e1; wK = 1e0; w = 1e9; K0 = 180000 50000 alpha = alpha0/j^9; N = 10
% Cdata = Nethcumcases(:,2);
tdata = Spaincumcases(:,1); % alpha0 = 1e1; wK = 1e0; w = 1e9; K0 = 180000 50000 alpha = alpha0/j^9; N = 10
Cdata = Spaincumcases(:,2);
...
num_it = 5;
N = 5;  % number of trig base functions divided by 2
% NP = 10620000; % population of Georgia was 10.62 million in 2019
% NP = 17134872; % population of Netherlands
NP = 46754778; % population of Spain
...
line1 = plot(tdata,curves_results,'Color', 'g');

hold on 

% line2 = plot(tdata,curves,'Color',[0.9290, 0.6940, 0.1250]);

line3 = plot(tdata,Cdata_inc,'ro');
set(line3,'LineWidth',2)
hold on

line4 = plot(tdata,mean(curves_results,2),'-k');
...
line1a = plot(tdata,curves_results_cum,'Color', 'c');

hold on 

% line2 = plot(tdata,curves,'Color',[0.9290, 0.6940, 0.1250]);

line3a = plot(tdata,Cdata,'ro');
set(line3,'LineWidth',2)
hold on

line4a = plot(tdata,mean(curves_results_cum,2),'-k');
...
line2 = plot(tdata,initial, 'Color', [0.8500, 0.3250, 0.0980]);
hold on
% fplot(@(t)r0(t),[a b],'-.dr');
% line10 = plot(tdata, bt,'-.^b');
line10 = plot(tdata, bt,'b');
hold on
line2a = plot(tdata,mean(bt,2),'-k');
...
line100 = plot(tdata, rp,'m');
hold on
line22a = plot(tdata,mean(rp,2),'-.^k');
set(line22a,'LineWidth',2)
hold on
line222 = fplot(@(t)horiz(t),[tdata(1) b],'Color',[0.4660, 0.6740, 0.1880]);
...
figure7 = figure;
subplot(2,2,1)
line77 = plot(tdata,curves_S, '-g');
hold on 
line77a = plot(tdata,mean(curves_S,2),'-k');
...
% hold on 
subplot(2,2,2)
line78 = plot(tdata,curves_I, 'Color', [0.6350, 0.0780, 0.1840]);
hold on 
line78a = plot(tdata,mean(curves_I,2),'-k');
...
% hold on 
subplot(2,2,3)
line79 = plot(tdata,curves_R, '-m');
hold on 
line79a = plot(tdata,mean(curves_R,2),'-k');
...
% hold on 
subplot(2,2,4)
line76 = plot(tdata,curves_D, '-c');
hold on 
line76a = plot(tdata,mean(curves_D,2),'-k');
...
Af(j,1) = 2*integral(@(x)fun1(x,j,ExpIncData),a,b)/(b-a);
Bf(j,1) = 2*integral(@(x)fun2(x,j,ExpIncData),a,b)/(b-a);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = fun1(x,j,ExpIncData)
global a b
       y = initial_guess_dIdt(x,ExpIncData).*cos(2*pi*j.*(x-a)./(b-a));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = fun2(x,j,ExpIncData)
global a b
       y = initial_guess_dIdt(x,ExpIncData).*sin(2*pi*j.*(x-a)./(b-a));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = initial_guess_I(t,ExpIncData)
global gamma tdata n

yy = zeros(n,1);
for k = 1:n
    yy(k) = exp(-gamma*tdata(k)).*sum(exp(gamma*tdata(1:k)).*ExpIncData(1:k));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = initial_guess_dIdt(t,ExpIncData)
global gamma tdata

pp = spline(tdata,ExpIncData);
yy = ppval(pp,t);

        y = yy - gamma*initial_guess_I(t,ExpIncData);

end   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = initial_guess_IntI(t,ExpIncData)
global  gamma tdata

Cdata_E = cumsum(ExpIncData);

%        y = integral(@(x)initial_guess_I(x),a,t);
         pp = spline(tdata,Cdata_E);
         yy = ppval(pp,t);
         y = (yy - initial_guess_I(t,ExpIncData))./gamma;
...
global gamma
    ...
    y = initial_guess_I(t,ExpIncData) + gamma*initial_guess_IntI(t,ExpIncData);
...
global gamma
    ...
    y = initial_guess_dIdt(t,ExpIncData) + gamma*initial_guess_I(t,ExpIncData);
